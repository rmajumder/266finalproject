    ...
    def run(self):
        ...
        with tf.Session() as sess:
            ...
            for i in range(self.n_iter):
                ...
                Small Test
                '''
                sall_preds, sall_labels = [], []
                for test in self.get_batch_data(ste_x, ste_y, ste_target_word, ste_tarmask, ste_loc, ste_mode,
                                                50, 1.0, 1.0, False):

                    _step, sty, spy, category, context = sess.run([global_step, true_y, pred_y, self.aspect_id, self.x],
                                                                feed_dict=test)
                    sall_preds.extend(spy)
                    sall_labels.extend(sty)
                
                sprecision, srecall, sf1, ssupport = metrics.precision_recall_fscore_support(sall_labels, sall_preds, average='macro')
